...
public class HTTPPoster {
	...
	public static String doPost(String urlString, ArrayList<NameValuePair> mPostData) {
		if (Constants.LOG_ENABLED) {
			Log.i(Constants.TAG, CLASS_TAG +  "doPost() : request url:" + urlString);
			Log.i(Constants.TAG, CLASS_TAG +  "doPost() : post data:" + mPostData);
		}
...
//		httppost.addHeader("Accept-Charset","UTF-8");
		try {
			...
			if (Constants.LOG_ENABLED) {
				Log.e(Constants.TAG, CLASS_TAG + "doPost(): UnsupportedEncodingException : " + e.getMessage());
				e.printStackTrace();
			}
		} catch (ClientProtocolException e) {
			if (Constants.LOG_ENABLED) {
				Log.e(Constants.TAG, CLASS_TAG + "doPost(): ClientProtocolException : " + e.getMessage());
				e.printStackTrace();
			}
		} catch (IOException e) {
			if (Constants.LOG_ENABLED) {
				Log.e(Constants.TAG, CLASS_TAG + "doPost(): IOException : " + e.getMessage());
				e.printStackTrace();
			}
		}
		if (Constants.LOG_ENABLED) {
			Log.i(Constants.TAG, CLASS_TAG + "doPost() : raw response:" + line);
		}
		return line;
	}
